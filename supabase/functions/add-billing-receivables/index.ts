...
serve(async (req)=>{
  ...
  try {
    ...
    if (!installmentId || !receivableIds || !Array.isArray(receivableIds) || receivableIds.length === 0) {
      return new Response(JSON.stringify({
        error: 'Invalid request data. Required: installmentId and receivableIds array'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (installmentError || !installment) {
      ...
      return new Response(JSON.stringify({
        error: `Installment not found: ${installmentId}. Please check that this installment exists in the database.`
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (receivablesError) {
      ...
      return new Response(JSON.stringify({
        error: `Error fetching receivables: ${receivablesError.message}`
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (!receivables || receivables.length === 0) {
      return new Response(JSON.stringify({
        error: 'No valid receivables found for the provided IDs'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (receivables.length !== receivableIds.length) {
      ...
      return new Response(JSON.stringify({
        error: `Some receivables do not exist: ${missingIds.join(', ')}`
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (wrongProjectReceivables.length > 0) {
      ...
      return new Response(JSON.stringify({
        error: `Some receivables do not belong to the project: ${wrongProjectReceivables.map((r)=>r.id).join(', ')}`
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (!diaCobranca) {
      return new Response(JSON.stringify({
        error: 'Missing payment plan settings data needed for nova_data_vencimento calculation'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (billingReceivables.length > 0) {
      ...
      const { data: insertedData, error: insertError } = await supabase.from('billing_receivables').insert(billingReceivables).select('id, installment_id, receivable_id, nova_data_vencimento, created_at');
      if (insertError) {
        console.error('Error inserting billing receivables:', insertError);
        return new Response(JSON.stringify({
          error: `Error inserting billing receivables: ${insertError.message}`
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
    ...
    const { data: allBillingReceivables, error: allBrError } = await supabase.from('billing_receivables').select(`
        receivable_id,
        receivables (
          amount
        )
    ...
    if (allBrError) {
      ...
      return new Response(JSON.stringify({
        error: `Error calculating total amount: ${allBrError.message}`
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    const totalAmount = (allBillingReceivables || []).reduce((sum, item)=>sum + parseFloat(item.receivables.amount), 0);
    ...
    try {
      ...
      const { data: settings, error: settingsError } = await supabase.from('payment_plan_settings').select('teto_fundo_reserva').eq('id', installment.payment_plan_settings_id).single();
      if (settingsError) {
      ...
      const { data: anticipation, error: anticipationError } = await supabase.from('anticipation_requests').select('valor_total').eq('id', anticipationRequestId).single();
      ...
      for (const inst of installments){
        ...
        const saldoDevedor = inst.numero_parcela === 0 ? saldoDevedorInitial - inst.pmt : Math.max(0, installments[installments.findIndex((i)=>i.id === inst.id) - 1]?.saldo_devedor - inst.pmt);
        ...
        if (inst.numero_parcela === 0) {
          ...
          await supabase.from('payment_plan_installments').update({
