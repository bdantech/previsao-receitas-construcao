...
async function handleGetBoletos(serviceClient, filters, corsHeaders) {
  ...
  let query = serviceClient.from('boletos').select(`
      id,
...
async function handleGetAvailableBillingReceivables(serviceClient, data, corsHeaders) {
  ...
  try {
    ...
    const indexIds = transformedData.filter((item)=>item.index_id).map((item)=>item.index_id);
    if (indexIds.length > 0) {
      const { data: indexesData, error: indexesError } = await serviceClient.from('indexes').select('id, name').in('id', indexIds);
...
async function handleCreateBoletos(serviceClient, billingReceivableIds, corsHeaders) {
  if (!billingReceivableIds || !Array.isArray(billingReceivableIds) || billingReceivableIds.length === 0) {
    return new Response(JSON.stringify({
      error: 'Billing receivable IDs are required'
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
  ...
  for (const brId of billingReceivableIds){
    try {
      ...
      const { data: existingBoleto } = await serviceClient.from('boletos').select('id').eq('billing_receivable_id', brId).maybeSingle();
      if (existingBoleto) {
      ...
      const { data: billingReceivableData, error: brQueryError } = await serviceClient.from('billing_receivables').select(`
          id,
          nova_data_vencimento,
          receivable_id,
          installment_id,
      ...
      const { nova_data_vencimento, receivables, payment_installments } = billingReceivableData;
      if (!receivables) {
      ...
      // Create the boleto
      const { data: boleto, error: createError } = await serviceClient.from('boletos').insert({
...
async function handleUpdateBoleto(serviceClient, boletoId, updateData, corsHeaders) {
  ...
  const { data: currentBoleto, error: getError } = await serviceClient.from('boletos').select('*').eq('id', boletoId).single();
  if (getError) {
    console.error(`Error fetching boleto ${boletoId}:`, getError);
    return new Response(JSON.stringify({
      error: `Boleto not found: ${getError.message}`
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
...
async function handleDeleteBoleto(serviceClient, boletoId, corsHeaders) {
  ...
  const { error: deleteError } = await serviceClient.from('boletos').delete().eq('id', boletoId);
  if (deleteError) {
