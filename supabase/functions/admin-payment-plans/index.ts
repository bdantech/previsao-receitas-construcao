      ...
      case 'createPaymentPlan':
        {
          ...
          const { data: anticipation, error: anticipationError } = await supabase.from('anticipation_requests').select('project_id').eq('id', anticipationRequestId).single();
          ...
          const { data: settings, error: settingsError } = await supabase.from('payment_plan_settings').insert({
          ...
          const { data: calResult, error: calError } = await supabase.rpc('calculate_payment_plan_installments', {
      ...
      case 'getPaymentPlans':
        {
          ...
          let query = supabase.from('payment_plan_settings').select(`
            id, 
            dia_cobranca, 
            teto_fundo_reserva,
            anticipation_request_id,
      ...
      case 'getInstallmentReceivables':
        {
          ...
          // Get PMT receivables
          const { data: pmtReceivables, error: pmtError } = await supabase.from('pmt_receivables').select(`
            id,
            receivable_id,
            receivables (
              id,
          ...
          const { data: billingReceivables, error: billingError } = await supabase.from('billing_receivables').select(`
            id,
            receivable_id,
      ...
      case 'getEligibleBillingReceivables':
        {
          ...
          const { data: installment, error: instError } = await supabase.from('payment_plan_installments').select('data_vencimento').eq('id', installmentId).single();
          if (instError) {
          ...
          const { data: existingBillingReceivables, error: existingError } = await supabase.from('billing_receivables').select('receivable_id');
          if (existingError) {
          ...
          const existingReceivableIds = (existingBillingReceivables || []).map((item)=>item.receivable_id).filter((id)=>id !== null);
          ...
          let query = supabase.from('receivables').select(`
            ...
            description,
            status
          `).eq('project_id', paymentPlan.project_id).gte('due_date', firstDayOfMonth).lte('due_date', lastDayOfMonth).not('status', 'eq', 'reprovado');
      ...
      case 'updateBillingReceivables':
        {
          ...
          const { data: receivables, error: receivablesError } = await supabase.from('receivables').select('id, amount, due_date, project_id').in('id', receivableIds);
          if (receivablesError) {
          ...
          console.log(`Found ${receivables.length} receivables out of ${receivableIds.length} requested:`, receivables.map((r)=>({
              id: r.id,
          ...
          const { data: existingLinks, error: existingLinksError } = await supabase.from('billing_receivables').select('receivable_id').eq('installment_id', installmentId).in('receivable_id', receivableIds);
          ...
          const { error: deleteError } = await supabase.from('billing_receivables').delete().eq('installment_id', installmentId);
          if (deleteError) {
          ...
          const { data: inserted, error: insertError } = await supabase.from('billing_receivables').insert(billingReceivables).select('id, installment_id, receivable_id, nova_data_vencimento, created_at');
          if (insertError) {
          ...
          const { data: verificationData, error: verificationError } = await supabase.from('billing_receivables').select('id, receivable_id, nova_data_vencimento').eq('installment_id', installmentId);
          ...
          const { data: updatedInstallment, error: updateError } = await supabase.from('payment_plan_installments').update({
            recebiveis: totalAmount
          }).eq('id', installmentId).select('*');
          if (updateError) {
          ...
          try {
            ...
            const { error: calcError } = await supabase.rpc('calculate_payment_plan_installments', {
          ...
          const { data: createdBillingReceivables, error: fetchError } = await supabase.from('billing_receivables').select(`
            id,
            receivable_id,
            nova_data_vencimento,
            created_at,
      ...
      case 'removeBillingReceivable':
        {
          ...
          `).eq('id', billingReceivableId).eq('installment_id', installmentId).single();
          if (brError) {
          ...
          const { error: deleteError } = await supabase.from('billing_receivables').delete().eq('id', billingReceivableId).eq('installment_id', installmentId);
          if (deleteError) {
          ...
          const { data: remainingReceivables, error: remainingError } = await supabase.from('billing_receivables').select(`
            receivable_id,
            receivables (
              amount
            )
      ...
      case 'deletePaymentPlan':
        {
          ...
          // Delete installments
          const { error: installmentsError } = await supabase.from('payment_plan_installments').delete().eq('payment_plan_settings_id', paymentPlanId);
          if (installmentsError) {
            throw new Error(`Error deleting installments: ${installmentsError.message}`);
          }
          // Delete payment plan settings
          const { error: settingsError } = await supabase.from('payment_plan_settings').delete().eq('id', paymentPlanId);
          if (settingsError) {
      ...
      case 'updatePaymentPlanSettings':
        {
          ...
          const { data: updatedSettings, error: updateError } = await supabase.from('payment_plan_settings').update({
            index_id: indexId,
            adjustment_base_date: adjustmentBaseDate
          }).eq('id', paymentPlanId).select('*').single();
          if (updateError) {
