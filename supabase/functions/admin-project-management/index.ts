...
console.log("Admin project management service active");
...
serve(async (req)=>{
  ...
  try {
    ...
    if (userRole !== "admin") {
      return new Response(JSON.stringify({
        error: "This endpoint is for admin users only"
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    const { data: projects, error: projectsError } = await supabaseAdmin.from("projects").select(`
        *,
