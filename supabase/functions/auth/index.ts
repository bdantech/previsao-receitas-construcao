  ...
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      ...
      return new Response(JSON.stringify({
        error: 'As variáveis de ambiente SUPABASE_URL e SUPABASE_ANON_KEY são obrigatórias'
      }), {
        headers: {
          ...corsHeaders,
    ...
    if (!adminSupabase) {
      ...
      return new Response(JSON.stringify({
        error: 'Configuração do servidor incompleta: chave de serviço indisponível'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    try {
      ...
      return new Response(JSON.stringify({
        error: 'Formato de dados inválido. Por favor, verifique seu envio.'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (action === 'register_company_user') {
      ...
      if (!email || !password || !companyData) {
        return new Response(JSON.stringify({
          error: 'Email, senha e dados da empresa são obrigatórios para criar uma conta'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      if (!companyData.name || !companyData.cnpj) {
        return new Response(JSON.stringify({
          error: 'Nome da empresa e CNPJ são obrigatórios'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      if (!cnpjRegex.test(companyData.cnpj)) {
        ...
        return new Response(JSON.stringify({
          error: 'CNPJ inválido. Use o formato XX.XXX.XXX/XXXX-XX ou 14 dígitos sem pontuação.'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      try {
        ...
        if (existingUser.users.some((u)=>u.email === email)) {
          return new Response(JSON.stringify({
            error: 'Este email já está em uso. Por favor, tente outro ou faça login.'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: existingCompany, error: existingCompanyError } = await adminSupabase.from('companies').select('id').eq('cnpj', companyData.cnpj).single();
        if (existingCompanyError && existingCompanyError.code !== 'PGRST116') {
          ...
          return new Response(JSON.stringify({
            error: 'Erro ao verificar empresa existente. Por favor, tente novamente.'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        if (existingCompany) {
          return new Response(JSON.stringify({
            error: 'Já existe uma empresa cadastrada com este CNPJ. Verifique os dados ou entre em contato com suporte.'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: companyResult, error: companyError } = await adminSupabase.from('companies').insert({
        ...
        const { error: relationError } = await adminSupabase.from('user_companies').insert({
          user_id: userId,
        ...
        if (relationError) {
          ...
          return new Response(JSON.stringify({
            error: `Erro ao vincular usuário à empresa: ${relationError.message}`
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: existingDocs, error: existingDocsError } = await adminSupabase.from('documents').select('id').eq('resource_type', 'company').eq('resource_id', companyResult.id);
        if (existingDocsError) {
          ...
          const { data: docTypes, error: docTypesError } = await adminSupabase.from('document_types').select('id, name').eq('resource', 'company');
          if (docTypesError) {
            ...
            try {
              ...
              const { error: docsError } = await adminSupabase.from('documents').insert(documentInserts);
              if (docsError) {
    ...
    if (action === 'register_admin_user') {
      ...
      try {
        const { data: currentUserProfile, error: profileError } = await adminSupabase.from('profiles').select('role').eq('id', user.id).single();
        if (profileError) {
    ...
    if (action === 'login') {
      ...
      try {
        ...
        const { data: profile, error: profileError } = await adminSupabase.from('profiles').select('role').eq('id', data.user.id).single();
        if (profileError) {
    ...
    return new Response(JSON.stringify({
      error: `Erro no servidor: ${error.message}. Por favor, tente novamente mais tarde.`
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
