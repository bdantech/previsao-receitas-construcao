...
async function handleCalculateValorLiquido(supabaseClient, serviceClient, data, corsHeaders) {
  ...
  if (!receivableIds || !Array.isArray(receivableIds) || receivableIds.length === 0) {
    return new Response(JSON.stringify({
      error: 'At least one receivable ID is required'
    }), {
      headers: {
        ...corsHeaders,
        'Content-Type': 'application/json'
  ...
  try {
    ...
    const { data: result, error } = await serviceClient.rpc('calculate_anticipation_valor_liquido', {
    ...
    const { data: receivables, error: receivablesError } = await supabaseClient.from('receivables').select(`
        id,
        amount,
        due_date,
        buyer_name,
        buyer_cpf,
        description
      `).in('id', receivableIds).eq('status', 'elegivel_para_antecipacao');
    if (receivablesError) {
      console.error('Error fetching receivables:', receivablesError);
      throw receivablesError;
    }
    // Get the credit analysis for the company
    const { data: creditAnalysis, error: creditAnalysisError } = await supabaseClient.from('company_credit_analysis').select(`
        interest_rate_180,
        interest_rate_360,
        interest_rate_720,
...
async function handleGetReceivablesForAnticipation(supabaseClient, serviceClient, data, corsHeaders) {
  ...
  const { data: projects, error: projectsError } = await supabaseClient.from('projects').select('id').eq('company_id', companyId);
  if (projectsError) {
...
async function handleCreateAnticipation(supabaseClient, serviceClient, data, corsHeaders, userId) {
  const { companyId, projectId, receivableIds, valorTotal, valorLiquido, taxaJuros180, taxaJuros360, taxaJuros720, taxaJurosLongoPrazo, tarifaPorRecebivel } = data;
  ...
  try {
    ...
    const { data: anticipation, error: anticipationError } = await serviceClient.from('anticipation_requests').insert({
    ...
    const { error: receivablesLinkError } = await serviceClient.from('anticipation_receivables').insert(anticipationReceivables);
    ...
    const { error: updateReceivablesError } = await serviceClient.from('receivables').update({
      status: 'antecipado'
    }).in('id', receivableIds);
...
async function handleGetAnticipations(supabaseClient, data, corsHeaders) {
  ...
  try {
    let query = supabaseClient.from('anticipation_requests').select(`
        id,
        project_id,
        valor_total,
        valor_liquido,
...
async function handleGetAnticipationReceivables(supabaseClient, data, corsHeaders) {
  ...
  try {
    const { data: anticipationReceivables, error } = await supabaseClient.from('anticipation_receivables').select(`
        id,
        anticipation_id,
        receivable_id,
...
async function handleGetAnticipationDetails(supabaseClient, data, corsHeaders) {
  ...
  try {
    ...
    const { data: anticipation, error: anticipationError } = await supabaseClient.from('anticipation_requests').select(`
        *,
        projects:project_id (
          name,
          cnpj
    ...
    const { data: receivablesResponse, error: receivablesError } = await handleGetAnticipationReceivables(supabaseClient, {
      anticipationId
    }, {}).then((res)=>res.json());
    if (receivablesError) {
