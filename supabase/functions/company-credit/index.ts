...
serve(async (req)=>{
  ...
  try {
    ...
    let userCompanies = [];
    if (!companyId) {
      const { data: companies, error: companiesError } = await supabase.from('user_companies').select('company_id').eq('user_id', user.id);
      if (companiesError) {
        return new Response(JSON.stringify({
      ...
      if (userCompanies.length === 0) {
        return new Response(JSON.stringify({
          error: 'No companies associated with this user'
        }), {
          status: 404,
          headers: {
            ...corsHeaders,
    ...
    switch(action){
      case 'list':
        ...
        if (companyId) {
          ...
          const { count, error: accessError } = await supabase.from('user_companies').select('*', {
            count: 'exact',
            head: true
          ...
          const { data, error: listError } = await supabase.from('company_credit_analysis').select(`
              *,
          ...
          const { data, error: listError } = await supabase.from('company_credit_analysis').select(`
              *,
              companies:company_id (name, cnpj)
            `).in('company_id', userCompanies).order('created_at', {
            ascending: false
          });
          result = data;
          error = listError;
      ...
      case 'get_active':
        ...
        if (companyId) {
          ...
          const { data, error: getError } = await supabase.from('company_credit_analysis').select(`
              *,
              companies:company_id (name, cnpj)
          ...
          const { data, error: getError } = await supabase.from('company_credit_analysis').select(`
              *,
              companies:company_id (name, cnpj)
            `).in('company_id', userCompanies).eq('status', 'Ativa');
          result = data;
          error = getError;
        }
        break;
