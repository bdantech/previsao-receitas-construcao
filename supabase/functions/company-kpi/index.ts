...
serve(async (req) => {
  ...
  try {
    ...
    const { data: userCompany, error: userCompanyError } = await serviceClient
      .from('user_companies')
      .select('company_id')
      .eq('user_id', user.id)
    ...
    const totalBuyers = new Set(
      projects?.flatMap(project => project.project_buyers.map(buyer => buyer.id))
    ).size;

    // Get receivables by status
    const { data: receivables, error: receivablesError } = await serviceClient
      .from('receivables')
    ...
    const { data: anticipations, error: anticipationsError } = await serviceClient
      .from('anticipations')
      .select('status, amount')
      .in('project_id', projects?.map(p => p.id) || []);
    ...
    const { data: boletos, error: boletosError } = await serviceClient
      .from('boletos')
      .select('status, amount')
      .in('project_id', projects?.map(p => p.id) || []);
