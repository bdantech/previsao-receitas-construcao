  ...
  try {
    ...
    switch (action) {
      ...
      case 'removeBillingReceivable': {
        ...
        console.log(`Recalculating payment plan for settings_id ${installmentCheck.payment_plan_settings_id}`);
        const { error: calcError } = await supabase.rpc(
          'calculate_payment_plan_installments',
          { p_payment_plan_settings_id: installmentCheck.payment_plan_settings_id }
        );
