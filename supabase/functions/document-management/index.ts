...
serve(async (req)=>{
  ...
  try {
    ...
    if (action === 'getDocumentSignedUrl') {
      ...
      const { data, error } = await serviceClient.storage.from('documents').createSignedUrl(filePath, 60); // URL valid for 60 seconds
      if (error) {
    ...
    if (requestData.action === 'getDocuments') {
      ...
      const { data: documents, error } = await supabaseClient.from('documents').select('*').eq('resource_type', resourceType).eq('resource_id', resourceId);
      if (error) {
    ...
    if (requestData.action === 'uploadFile') {
      ...
      const { data: uploadData, error: uploadError } = await serviceClient.storage.from('documents').upload(storagePath, uint8Array, {
      ...
      if (documentId) {
        // Updating a document
        const { data: documentData, error: documentError } = await serviceClient.from('documents').update({
        ...
        try {
          ...
          const { data: buyerData, error: buyerError } = await serviceClient.from('project_buyers').update({
