  ...
  try {
    ...
    switch(action){
      ...
      case 'getIndexes':
        ...
        const { data: indexes, error: indexesError } = await supabase.from('indexes').select('*').order('name');
        if (indexesError) {
      ...
      case 'createIndex':
        ...
        if (!isAdmin) {
          return new Response(JSON.stringify({
            error: 'Only admin users can create indexes'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: newIndex, error: createError } = await supabase.from('indexes').insert({
          name: data.name,
      ...
      case 'updateIndex':
        ...
        if (!isAdmin) {
          return new Response(JSON.stringify({
            error: 'Only admin users can update indexes'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: updatedIndex, error: updateError } = await supabase.from('indexes').update({
          name: data.name,
          description: data.description
      ...
      case 'deleteIndex':
        ...
        if (!isAdmin) {
          return new Response(JSON.stringify({
            error: 'Only admin users can delete indexes'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { error: deleteError } = await supabase.from('indexes').delete().eq('id', data.id);
        if (deleteError) {
      ...
      case 'getIndexUpdates':
        ...
        const { data: updates, error: updatesError } = await supabase.from('indexes_update').select('*').eq('index_id', data.indexId).order('reference_month');
        if (updatesError) {
          console.error('Error fetching index updates:', updatesError);
          return new Response(JSON.stringify({
            error: 'Error fetching index updates'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      case 'createIndexUpdate':
        ...
        if (!isAdmin) {
          return new Response(JSON.stringify({
            error: 'Only admin users can create index updates'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        try {
          ...
          const { data: newUpdate, error: createUpdateError } = await supabase.from('indexes_update').insert({
            index_id: data.indexId,
            reference_month: referenceDate.toISOString(),
            monthly_adjustment: data.monthlyAdjustment
          }).select().single();
          if (createUpdateError) {
            ...
            return new Response(JSON.stringify({
              error: 'Error creating index update: ' + createUpdateError.message
            }), {
              headers: {
                ...corsHeaders,
                'Content-Type': 'application/json'
      ...
      case 'updateIndexUpdate':
        ...
        if (!isAdmin) {
          return new Response(JSON.stringify({
            error: 'Only admin users can update index updates'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: updatedUpdate, error: updateUpdateError } = await supabase.from('indexes_update').update({
          monthly_adjustment: data.monthlyAdjustment
        }).eq('id', data.id).select().single();
        if (updateUpdateError) {
          console.error('Error updating index update:', updateUpdateError);
          return new Response(JSON.stringify({
            error: 'Error updating index update'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      case 'deleteIndexUpdate':
        ...
        if (!isAdmin) {
          return new Response(JSON.stringify({
            error: 'Only admin users can delete index updates'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { error: deleteUpdateError } = await supabase.from('indexes_update').delete().eq('id', data.id);
        if (deleteUpdateError) {
          console.error('Error deleting index update:', deleteUpdateError);
          return new Response(JSON.stringify({
            error: 'Error deleting index update'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      case 'getIndexesForSelect':
        ...
        const { data: indexesForSelect, error: indexesSelectError } = await supabase.from('indexes').select('id, name').order('name');
        if (indexesSelectError) {
          console.error('Error fetching indexes for select:', indexesSelectError);
          return new Response(JSON.stringify({
            error: 'Error fetching indexes for select'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      case 'calculateCompoundAdjustment':
        ...
        if (!indexId || !startDate || !endDate) {
          return new Response(JSON.stringify({
            error: 'Missing required parameters: indexId, startDate, endDate'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        const { data: adjustments, error: adjustmentsError } = await supabase.from('indexes_update').select('reference_month, monthly_adjustment').eq('index_id', indexId).gte('reference_month', start.toISOString()).lte('reference_month', end.toISOString()).order('reference_month');
        if (adjustmentsError) {
