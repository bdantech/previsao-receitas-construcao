  ...
  try {
    ...
    if (action === 'list') {
      ...
      if (projectId) {
        ...
        if (!isAdmin) {
          const { data: userProject, error: projectError } = await serviceClient.from('projects').select('id, company_id').eq('id', projectId).single();
          if (projectError) {
            console.error('Project check error:', projectError);
            return new Response(JSON.stringify({
              error: 'Project not found or you lack access'
            }), {
              headers: {
                ...corsHeaders,
                'Content-Type': 'application/json'
          ...
          const { data: userCompanies, error: companyError } = await serviceClient.from('user_companies').select('company_id').eq('user_id', user.id).eq('company_id', userProject.company_id);
        ...
        const { data: userCompanies, error: companyError } = await serviceClient.from('user_companies').select('company_id').eq('user_id', user.id);
        if (companyError) {
        ...
        const { data: companyProjects, error: projectsError } = await serviceClient.from('projects').select('id').in('company_id', companyIds);
        if (projectsError) {
      ...
      const { data: buyers, error: buyersError } = await query.order('created_at', {
        ascending: false
      });
      if (buyersError) {
    ...
    if (action === 'get' && buyerId) {
      const { data: buyer, error: buyerError } = await serviceClient.from('project_buyers').select('*, projects(*)').eq('id', buyerId).single();
      if (buyerError) {
        console.error('Project buyer fetch error:', buyerError);
        if (buyerError.code === 'PGRST116') {
          return new Response(JSON.stringify({
            error: 'Project buyer not found or you lack access'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      if (!isAdmin) {
        ...
        const { data: projectCompany, error: projectError } = await serviceClient.from('projects').select('company_id').eq('id', buyer.project_id).single();
        if (projectError) {
        ...
        if (!companyIds.includes(projectCompany.company_id)) {
          return new Response(JSON.stringify({
            error: 'You do not have permission to access this buyer'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
    ...
    if (action === 'create' && projectId && buyerData) {
      ...
      if (!isAdmin) {
        ...
        const { data: project, error: projectError } = await serviceClient.from('projects').select('company_id').eq('id', projectId).single();
        if (projectError) {
          console.error('Project fetch error:', projectError);
          throw projectError;
        }
        const { data: userCompanies, error: companyError } = await serviceClient.from('user_companies').select('company_id').eq('user_id', user.id).eq('company_id', project.company_id);
        if (companyError || !userCompanies.length) {
          return new Response(JSON.stringify({
            error: 'You do not have permission to create buyers for this project'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      if (!full_name || !cpf) {
        return new Response(JSON.stringify({
          error: 'Required fields: full_name, cpf'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      const { data: buyer, error: buyerError } = await serviceClient.from('project_buyers').insert({
    ...
    if (action === 'update' && buyerId && buyerData) {
      ...
      const { data: buyer, error: buyerFetchError } = await serviceClient.from('project_buyers').select('project_id').eq('id', buyerId).single();
      if (buyerFetchError) {
      ...
      // For company users, verify they have access to this buyer's project
      if (!isAdmin) {
        const { data: project, error: projectError } = await serviceClient.from('projects').select('company_id').eq('id', buyer.project_id).single();
        if (projectError) {
          console.error('Project fetch error:', projectError);
          throw projectError;
        }
        ...
        for (const field of requestedFields){
          if (!allowedFields.includes(field)) {
            return new Response(JSON.stringify({
              error: `Company users cannot update the "${field}" field`
            }), {
              headers: {
                ...corsHeaders,
                'Content-Type': 'application/json'
      ...
      const { data: updatedBuyer, error: buyerError } = await serviceClient.from('project_buyers').update(buyerData).eq('id', buyerId).select().single();
      if (buyerError) {
