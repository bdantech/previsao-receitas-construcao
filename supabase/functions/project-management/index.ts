...
serve(async (req)=>{
  ...
  try {
    ...
    if (endpoint === 'projects' && method === 'GET') {
      ...
      const { data: projects, error: projectsError } = await query.order('created_at', {
        ascending: false
      });
      if (projectsError) {
    ...
    if (endpoint === 'projects' && method === 'POST') {
      ...
      if (!name || !cnpj || !company_id || !initial_date) {
        return new Response(JSON.stringify({
          error: 'Required fields: name, cnpj, company_id, initial_date'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
      ...
      if (!isAdmin) {
        const { data: userCompany, error: userCompanyError } = await supabaseClient.from('user_companies').select('company_id').eq('user_id', user.id).eq('company_id', company_id).single();
        if (userCompanyError) {
          console.error('User company verification error:', userCompanyError);
          return new Response(JSON.stringify({
            error: 'You can only create projects for your company'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
      ...
      const { data: project, error: projectError } = await supabaseClient.from('projects').insert({
        name,
        cnpj,
        company_id,
        initial_date,
    ...
    if (endpoint && method === 'PUT') {
      const projectId = endpoint.startsWith('projects/') ? endpoint.split('/')[1] // Handle case where "projects/" is still included for backward compatibility
      ...
      let query = supabaseClient.from('projects').update(updates).eq('id', projectId);
      // If not admin, verify user has access to this project
      if (!isAdmin) {
        const { data: userCompany, error: userCompanyError } = await supabaseClient.from('user_companies').select('company_id').eq('user_id', user.id).single();
        if (userCompanyError) {
      ...
      const { data: project, error: projectError } = await query.select().single();
      if (projectError) {
