...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Verify credentials
    const { data: credentialData, error: credentialError } = await supabase.from('company_api_credentials').select('company_id').eq('client_id', clientId).eq('client_secret', clientSecret).eq('active', true).single();
    if (credentialError || !credentialData) {
      return new Response(JSON.stringify({
        error: 'Credenciais inválidas ou inativas'
      }), {
        status: 401,
        headers: {
          ...corsHeaders,
    ...
    if (req.method === 'POST') {
      ...
      const { data: projectData, error: projectError } = await supabase.from('projects').select('id').eq('id', reqData.project_id).eq('company_id', companyId).single();
      if (projectError || !projectData) {
        return new Response(JSON.stringify({
          error: 'Projeto não encontrado ou não pertence a esta empresa'
        }), {
          status: 404,
          headers: {
            ...corsHeaders,
      ...
      for (const receivable of reqData.receivables){
        ...
        try {
          ...
          const amount = typeof receivable.amount === 'string' ? parseFloat(receivable.amount.replace(/[^\d.,]/g, '').replace(',', '.')) : Number(receivable.amount);
          ...
          const { data: statusData, error: statusError } = await supabase.rpc('get_initial_receivable_status', {
      ...
      if (receivablesToInsert.length > 0) {
        const { data: insertedData, error: insertError } = await supabase.from('receivables').insert(receivablesToInsert).select();
        if (insertError) {
          return new Response(JSON.stringify({
