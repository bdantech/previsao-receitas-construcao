...
serve(async (req)=>{
  ...
  try {
    ...
    if (endpoint === 'receivables' && method === 'GET') {
      try {
        let query = adminSupabase.from('receivables').select(`
            id,
            project_id,
            buyer_name,
            buyer_cpf,
        ...
        if (!isAdmin) {
          // Get user's company
          const { data: userCompany, error: companyError } = await adminSupabase.from('user_companies').select('company_id').eq('user_id', user.id).single();
          if (companyError) {
            console.error('Error getting user company:', companyError);
            throw new Error('Failed to verify user company');
          }
          // Get company's projects
          const { data: projects, error: projectsError } = await adminSupabase.from('projects').select('id').eq('company_id', userCompany.company_id);
          if (projectsError) {
    ...
    if (endpoint === 'receivables' && method === 'POST') {
      try {
        ...
        if (!projectId || !buyerName || !buyerCpf || !amount || !dueDate) {
          return new Response(JSON.stringify({
            error: 'Required fields: projectId, buyerName, buyerCpf, amount, dueDate'
          }), {
            headers: {
              ...corsHeaders,
              'Content-Type': 'application/json'
        ...
        if (!isAdmin) {
          ...
          const { data: projectData, error: projectError } = await adminSupabase.from('projects').select('company_id').eq('id', projectId).single();
          if (projectError) {
          ...
          // Get user's company
          const { data: userCompanies, error: userCompanyError } = await adminSupabase.from('user_companies').select('company_id').eq('user_id', user.id);
          if (userCompanyError) {
            console.error('User company verification error:', userCompanyError);
            return new Response(JSON.stringify({
              error: 'Failed to verify user company access'
            }), {
              headers: {
                ...corsHeaders,
                'Content-Type': 'application/json'
          ...
          if (!hasAccess) {
            ...
            return new Response(JSON.stringify({
              error: 'You can only create receivables for your company projects'
            }), {
              headers: {
                ...corsHeaders,
                'Content-Type': 'application/json'
        ...
        const { data: existingBuyer, error: buyerCheckError } = await adminSupabase.from('project_buyers').select('id, full_name, buyer_status').eq('project_id', projectId).eq('cpf', buyerCpf).maybeSingle();
        if (buyerCheckError) {
        ...
        if (!existingBuyer) {
          ...
          const { data: newBuyer, error: createBuyerError } = await adminSupabase.from('project_buyers').insert({
        ...
        const { data: receivable, error: createError } = await adminSupabase.from('receivables').insert({
          project_id: projectId,
      ...
      try {
        ...
        if (!isAdmin) {
          ...
          if (userCompany.company_id !== projectCompanyId) {
            return new Response(JSON.stringify({
              error: 'You do not have access to this receivable'
            }), {
              headers: {
                ...corsHeaders,
                'Content-Type': 'application/json'
        ...
        return new Response(JSON.stringify({
          error: error.message || 'Failed to fetch receivable'
        }), {
          headers: {
            ...corsHeaders,
            'Content-Type': 'application/json'
    ...
    if (endpoint.startsWith('receivables/') && method === 'PUT') {
      try {
        ...
        const { data: receivable, error: getError } = await adminSupabase.from('receivables').select(`
            id,
            project_id,
            projects:project_id (
              company_id
        ...
        const { data: updatedReceivable, error: updateError } = await adminSupabase.from('receivables').update(updates).eq('id', receivableId).select().single();
        if (updateError) {
