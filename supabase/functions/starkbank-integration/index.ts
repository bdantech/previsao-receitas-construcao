...
serve(async (req)=>{
  ...
  try {
    ...
    // Get boleto data
    const { data: boleto, error: boletoError } = await adminSupabase.from('boletos').select('*').eq('id', boletoId).single();
    ...
    const { data: bankAccount, error: bankError } = await adminSupabase.from('bank_accounts').select('*').eq('project_id', boleto.project_id).single();
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return new Response(JSON.stringify({
          error: 'Failed to create boleto in Starkbank',
          details: errorData
        }), {
          headers: {
            ...corsHeaders,
      ...
      const pdfFileName = `boleto_${createdBoleto.id}.pdf`;
      const pdfFilePath = `boletos/${boletoId}/${pdfFileName}`;
      const { error: storageError } = await adminSupabase.storage.from('documents').upload(pdfFilePath, pdfBuffer, {
      ...
      const { error: updateError } = await adminSupabase.from('boletos').update(updateData).eq('id', boletoId);
      if (updateError) {
