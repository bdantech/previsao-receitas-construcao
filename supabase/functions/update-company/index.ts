...
serve(async (req) => {
  ...
  try {
    ...
    if (relationError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Erro ao verificar relação entre usuário e empresa' }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 500 
        }
    ...
    if (userCompanies.length === 0) {
      return new Response(
        JSON.stringify({ error: 'Usuário não autorizado a atualizar esta empresa' }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 403 
        }
    ...
    // Update company
    const { data: company, error: updateError } = await adminSupabase
      .from('companies')
      .update({
        name: updates.name,
    ...
    if (updateError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Erro ao atualizar empresa: ' + updateError.message }),
        { 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          status: 500 
        }
