...
serve(async (req)=>{
  ...
  try {
    ...
    if (userRole !== "company_user") {
      return new Response(JSON.stringify({
        error: "This endpoint is for company users only"
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    if (userCompaniesError) {
      return new Response(JSON.stringify({
        error: "Failed to fetch user's company",
    ...
    const { data: companies, error: companiesError } = await supabaseAdmin.from("companies").select("*").in("id", companyIds);
    if (companiesError) {
      return new Response(JSON.stringify({
